...
abstract class MyActionPropertyChangeListener<T extends JComponent> implements
	...
	/**
	 * PropertyChangeListener method. If the target has been gc'ed this will
	 * remove the <code>PropertyChangeListener</code> from the Action, otherwise
	 * this will invoke actionPropertyChanged.
	 */
	...
	private static class OwnedWeakReference<U extends JComponent> extends
		...
		OwnedWeakReference(U target, ReferenceQueue<? super U> queue,
				MyActionPropertyChangeListener<U> owner) {
			super(target, queue);
			this.owner = owner;
		}
	...
	/**
	 * Whether or not to reconfigure all action properties from the specified
	 * event.
	 */
